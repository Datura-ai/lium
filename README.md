...
- `lium topup card -a <USD> [--card <pm_id>]` - Charge a card saved on the account, with no browser; the API key needs the `billing` scope (not released: the platform switch is off). Sent once with an idempotency key; a lost answer or a 202 still-confirming exits 6 ("the charge may have gone through" / "still being confirmed") with the key to re-run with, never a bare retry
