...
Failure on a renter command that takes `--json` (`up`, `exec`, `describe`, `balance`, `whoami`, `audit`, `cp`, `init`; `fund`, `signup`, `topup currencies`, `topup create`, `keys create`, `keys list`, `keys show`, `keys scopes`, `workspaces list` and `workspaces members` too): stdout is empty, **stderr** holds one JSON object, the exit code is non-zero:
...
- `lium keys create <name> --json` returns the new key as the server sends it (the secret under `key`, printed this once) plus `workspace_name`; needs `lium workspaces login` first. Without `--scope` the key gets `read`, `rent` and `manage` — never `billing`, the money-moving scope, which is added only by `--scope billing` (a warning line goes to stderr first). `--daily-budget USD` / `--max-budget USD` cap what the key's pods may be billed per UTC day / in total, `--pod-visibility own|account` says whether the key sees only the pods it rented (default) or every pod of the account (budgets, visibility: lium-platform P235, not released).
- `lium keys list --json` returns a list of the workspace's key rows (`id`, `name`, `scopes`, `created_at`, `last_used`, and — `null` from a server before P235 — `daily_budget_usd`, `max_budget_usd`, `spent_today_usd`, `spent_total_usd`, `pod_visibility`) without the key material; needs `lium workspaces login` first.
- `lium keys show <name|id> --json` returns one key's row plus `can_do` (one line per scope the key holds, the server's description from `GET /keys/scopes`; the scope name alone on an older server) and `pods` (active pods rented through the key, `null` when `lium ps` could not be read); `lium keys scopes --json` returns every scope as `{"scope", "description", "route_families"}` (lium-platform P235, not released; older servers answer `not_found`).
- `lium ps --key <name|id> --format json` lists only the pods rented through that key (each row then carries `api_key_id` / `api_key_name`); `lium billing history [--key <name|id>] [--from YYYY-MM-DD] [--to YYYY-MM-DD] --format json` prints the ledger statement, `{"start_day", "end_day", "total", "pods": [...]}`, each pod with its `total`, `billed_seconds` and per-day `days`. A key name needs `lium workspaces login`; an id does not. A rent refused by the key's budget fails with the server's code `API_KEY_BUDGET_EXCEEDED` (exit 6; `data.window`, `data.budget_usd`, `data.spent_usd`), a route the key's scopes do not cover with `missing_scope` (exit 6, `data.scope`).
