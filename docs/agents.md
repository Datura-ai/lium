...
The balance a `--wait` watches is read with the same key that pays, so both are one account. Under a workspace (`-w`, `LIUM_WORKSPACE`, `lium workspaces use`) the workspace's key pays and the personal billing key is not used; with an exported `LIUM_API_KEY` the saved billing key is not used either (it may be another account's) — export `LIUM_BILLING_API_KEY` with it. A `--wait` that cannot read the balance first creates nothing (`balance_unreadable`, exit 3); a timeout is `credit_not_seen`, exit 3 after a page or invoice (`data.charged: null`, not known: keep waiting) and exit 6 after a card charge (`data.charged: true`: do not charge again).
