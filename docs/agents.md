...
- `lium init` without a terminal next to a saved key the API refuses (expired or revoked) opens no browser and exits 6 with `error.code` `saved_key_rejected`; the key is left in place. Recover with `lium init --api-key <key>`, or `lium init --force --no-browser` then `lium init --session <ID>` once a person approves the URL.
