...
- `lium init` without a terminal next to a saved key the API rejects (401, or a 403 saying the key's workspace is gone or its creator left it: expired or revoked) opens no browser and exits 6 with `error.code` `saved_key_rejected`; the key is left in place. Recover with `lium init --api-key <key>`, or `lium init --force --no-browser` then `lium init --session <ID>` once a person approves the URL; the old key stays until that session is approved. Any other 403 keeps the key with a warning and exits 0.
