...
| `ssh_connection_failed` | 4 | ssh could not connect to a RUNNING pod. From `lium up` and `lium ssh`, `data.ssh_port_answered` is `false` when the pod's SSH port sent no SSH banner in the 60 s they wait; the pod is left running. | Port answered: check `lium config get ssh.key_path` and `lium ssh-keys`. Port never answered: retry `lium ssh <huid>`, or `lium rm <huid>` and rent another node. |
