...
| `UUID_NOT_FOUND` | 1 | `host.uuid_not_found` | 5 |
  ...
  `auth.not_signed_in`), Ctrl-C (1 → 130, `input.interrupted`), `config set-password` without `--password`
...
| `input.interrupted` | 130 | Ctrl-C during a `lium provider` command in agent mode. |
