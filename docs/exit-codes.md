...
| `budget_exceeded` | 6 | 402 from the rent path: the API key's daily or total budget (`lium keys create --daily-budget / --max-budget`, `lium keys budget`) is reached, so a new rental through that key is refused and the billing tick stops the key's pods (lium-platform#630, not released; the server's own `error.code` is `API_KEY_BUDGET_EXCEEDED` and is the code you get when it sends one). The SDK raises `LiumBudgetExceededError` with `window`, `budget_usd`, `spent_usd` and `api_key_id` when the server stated them; the envelope's `data` repeats them. | `lium keys show <name>` shows the figures; raise or clear the budget with `lium keys budget <name>` (a signed-in session) or rent with another key. |
